...
STAGE_BOUNDARY_EXAMPLE_TEST = "S.30.2"          # Fig 1.4
...
def generate_raw_data_figures() -> None:
    ...
    print("Generating Fig 1.4 (stage-boundary example)...")
    test = experiment.tests[STAGE_BOUNDARY_EXAMPLE_TEST]
    out_path = plot_creep_stage_boundaries(
        test.time_series, test.strain_series, test.test_id, DIR_EXPERIMENTAL, style
    )
    print(f"  Saved: {out_path}")
...
def generate_modelling_figures_and_table() -> None:
    ...
    for quality in ["High", "Standard"]:
        fig, axes = plt.subplots(1, 3, figsize=(8.5, 5.2), sharex=True, sharey=True)
        legend_handles = [
        ...
        for col_idx, stress in enumerate([10.0, 20.0, 30.0]):
            ax = axes[col_idx]
            tests = grouped.get((quality, stress), [])
            ax.grid(True, linestyle="--", alpha=0.4)
            ax.set_xlabel("Time (s)")
            for i, t in enumerate(tests):
                c = palette[i % len(palette)]
                ax.scatter(t["time_s"], t["strain_measured"], alpha=0.6, s=10, color=c)
        ...
        out_path = DIR_MODELLING / f"Summary_{quality}_Combined.png"
        DIR_MODELLING.mkdir(parents=True, exist_ok=True)
        fig.savefig(out_path, dpi=300, bbox_inches="tight")
        plt.close(fig)
        print(f"  Saved: {out_path}")

    print("Generating Table 1.2 (fitted TLV parameters)...")
...
def generate_parameter_table() -> None:
    ...
    latex = [
        ...
        r"    \label{tab:fitted_tlv_params}", r"    \begin{tabular}{llcc}", r"        \toprule",
        r"        \textbf{Parameter} & \textbf{Units} & \textbf{High} & \textbf{Standard} \\", r"        \midrule",
    ]
    for symbol, units, key, f in param_rows:
        latex.append(f"        {symbol} & {units} & {fmt(high_attrs.get(key), f)} & {fmt(std_attrs.get(key), f)} \\\\")
    latex.extend([r"        \bottomrule", r"    \end{tabular}", r"\end{table}"])

    DIR_MODELLING.mkdir(parents=True, exist_ok=True)
    out_path = DIR_MODELLING / "fitted_tlv_params.tex"
